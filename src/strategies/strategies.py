...
class LowVolatilityDecileStrategy(Strategy):
    def generate_signals(self, market_data: dict[str, pd.DataFrame], compositions: list[str], date: datetime, end_date, frequency, rebalance_at, weights_type) -> dict[str, float]:
        ...
        low_volatility_decile = self.generate_weights(low_volatility_decile, volatilities, weights_type)
...
class HighVolatilityDecileStrategy(Strategy):
    def generate_signals(self, market_data: dict[str, pd.DataFrame], compositions: list[str], date: datetime, end_date, frequency, rebalance_at, weights_type) -> dict[str, float]:
        ...
        high_volatility_decile = self.generate_weights(high_volatility_decile, volatilities, weights_type)
...
class MidVolatilityDecileStrategy(Strategy):
    def generate_signals(self, market_data: dict[str, pd.DataFrame], compositions: list[str], date: datetime, end_date, frequency, rebalance_at, weights_type) -> dict[str, float]:
        ...
        mid_volatility_decile = self.generate_weights(mid_volatility_decile, volatilities, weights_type)
...
class VolatilityTimingStrategy(Strategy):  
    ...
    def generate_signals(self, market_data: dict[str, pd.DataFrame], compositions: list[str], date: datetime, end_date) -> dict[str, float]:
        low_decile, next_date = LowVolatilityDecileStrategy().generate_signals(market_data, compositions, date, end_date, self.frequency, self.rebalance_at, self.weights_type)
        high_decile, next_date = HighVolatilityDecileStrategy().generate_signals(market_data, compositions, date, end_date, self.frequency, self.rebalance_at, self.weights_type)
...
class VolatilityTimingStrategy2sided(Strategy):  
    ...
    def generate_signals(self, market_data: dict[str, pd.DataFrame], compositions: list[str], date: datetime, end_date) -> dict[str, float]:
        ...
        if self.ptf_hold[date] == 'High':
            ...
            return low_decile, next_date    
        mid_decile, next_date = MidVolatilityDecileStrategy().generate_signals(market_data, compositions, date, end_date, self.frequency, self.rebalance_at, self.weights_type)
